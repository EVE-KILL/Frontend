	...
	<div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 modal-overlay" on:click={handleOutsideClick}>
		<div class="bg-background-800 text-white rounded-lg p-8 w-full max-w-3xl mx-4 max-h-[90vh] overflow-y-auto relative">
			...
			<div class="mb-6">
				...
				<input type="text" bind:value={campaignName} class="block w-full border border-gray-600 rounded-lg py-1 px-4 bg-background-900 text-white" />
			</div>
			...
			<div class="mb-6">
				...
				<input
					...
					class="block w-full border border-gray-600 rounded-lg py-1 px-4 bg-background-900 text-white"
				/>
			</div>
			...
			{#each entities as entity, index}
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center mb-6 relative">
					...
					<div>
						...
						<select bind:value={entities[index].type} class="block w-full border border-gray-600 rounded-lg py-2 px-4 bg-background-900 text-white">
					...
					<div class="relative">
						...
						<div class="flex items-center border border-gray-600 rounded-lg py-2 px-4 bg-background-900 text-white">
							{#if entity.imageUrl}
							...
							<div class="absolute bg-background-800 rounded-lg shadow-lg -mt-1 w-full bottom-full mb-2">
								<ul class="max-h-64 overflow-y-auto">
									{#each entities[index].searchResults as result, resultIndex}
										<li
											class="flex items-center px-4 py-2 text-gray-300 cursor-pointer hover:bg-background-700 {entities[index].selectedIndex ===
											resultIndex
												? 'bg-background-600'
												: ''}"
					...
					<div>
						...
						{#if treatments[entities[index].type] && Array.isArray(treatments[entities[index].type])}
							<select
								...
								class="block w-full border border-gray-600 rounded-lg py-2 px-4 bg-background-900 text-white"
							>
			...
			{#each locations as location, index}
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center mb-6 relative">
					...
					<div>
						<label class="block text-gray-300 mb-2">Type</label>
						<select bind:value={locations[index].type} class="block w-full border border-gray-600 rounded-lg py-2 px-4 bg-background-900 text-white">
							...
							<div class="absolute bg-background-800 rounded-lg shadow-lg -mt-1 w-full bottom-full mb-2">
								<ul class="max-h-64 overflow-y-auto">
									{#each locations[index].searchResults as result, resultIndex}
										<li
											class="flex items-center px-4 py-2 text-gray-300 cursor-pointer hover:bg-background-700 {locations[index].selectedIndex ===
											resultIndex
												? 'bg-background-600'
												: ''}"
			...
			{#each timePeriods as timePeriod, index}
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center mb-6 relative">
					...
					<div>
						...
						<DateInput bind:value={timePeriod.from} class="block w-full border border-gray-600 rounded-lg py-2 px-4 bg-background-900 text-white" />
					</div>
