			...
			<tbody class="text-gray-300 text-sm">
				{#each killmails as kill (kill.killmail_id)}
					<tr
						...
						on:click={() => handleKillmailClick(kill.killmail_id)}
					>
						<td class="flex items-center space-x-2">
							<a href={`/kill/${kill.killmail_id}`}>
								<img
									src={`https://images.evetech.net/types/${kill.victim.ship_id}/render?size=64`}
							...
							<a href={`/corporation/${kill.victim.corporation_id}`}>
								<img
									src={`https://images.evetech.net/corporations/${kill.victim.corporation_id}/logo?size=64`}
									class="rounded w-16 h-16 m-1"
							...
							{#if kill.victim.alliance_id}
								<a href={`/alliance/${kill.victim.alliance_id}`}>
									<img
										src={`https://images.evetech.net/alliances/${kill.victim.alliance_id}/logo?size=64`}
										class="rounded w-16 h-16 m-1"
						...
						<td class="pl-5">
							<div class="text-sm">
								<a href={`/kill/${kill.killmail_id}`} class="text-blue-400"
									>{kill.victim.character_name} ({kill.victim.ship_name})</a
								><br />
								<a
									href={`/corporation/${kill.victim.corporation_id}`}
									class="text-blue-400">{kill.victim.corporation_name}</a
								><br />
								{#if kill.victim.alliance_name}
									<a
										href={`/alliance/${kill.victim.alliance_id}`}
										class="text-blue-400">{kill.victim.alliance_name}</a
									><br />
								{/if}
							</div>
						</td>
