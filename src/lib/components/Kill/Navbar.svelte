		...
		<ul class="flex space-x-4 justify-between w-full">
			...
			<li class="relative">
				...
				{#if isExternalDropDownOpen}
					<ul
						class="absolute left-0 mt-2 w-48 bg-gray-800 rounded-md shadow-lg z-10"
						on:mouseenter={openExternalDropdown}
						on:mouseleave={closeExternalDropdown}
					>
						...
						<li>
							<a
								href="https://evemaps.dotlan.net/system/{killmail.system_name}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.system_name}</a
							>
						</li>
						<li>
							<a
								href="https://evemaps.dotlan.net/region/{killmail.region_name}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.region_name}</a
							>
						</li>
						<li class="px-4 py-2 text-sm text-gray-300">EVEEye</li>
						<li>
							<a
								href="https://eveeye.com/?m={killmail.region_name}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.region_name}</a
							>
						</li>
						...
						<li>
							<a
								href="https://evemissioneer.com/s/{killmail.system_id}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.system_name}</a
							>
						</li>
						<li>
							<a
								href="https://evemissioneer.com/r/{killmail.region_id}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.region_name}</a
							>
						</li>
						{#if killmail.region_id >= 11000001 && killmail.region_id <= 11000033}
							...
							<li>
								<a
									href="http://anoik.is/systems/{killmail.system_name}"
									target="_blank"
						...
						<li>
							<a
								...
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">ESF</a
							>
						</li>
						<li class="px-4 py-2 text-sm text-gray-300">EVE Ref</li>
						<li>
							<a
								href="https://everef.net/groups/{killmail.victim.ship_group_id}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700"
						...
						<li>
							<a
								href="https://everef.net/type/{killmail.victim.ship_id}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700"
						...
						<li>
							<a
								href="https://www.jita.space/system/{killmail.system_id}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.system_name}</a
							>
						</li>
						<li>
							<a
								href="https://www.jita.space/region/{killmail.region_id}"
								target="_blank"
								class="block px-4 py-2 hover:bg-gray-700">{killmail.region_name}</a
							>
						</li>
			...
			{#if inBattle}
				<li class="ml-auto">
					<a
						href="/battle/kill/{killmail.killmail_id}"
						class="text-white hover:text-gray-400">Battle Info</a
					>
				</li>
			{/if}
		</ul>
