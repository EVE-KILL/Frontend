	...
	<table class="table-auto min-w-full bg-semi-transparent bg-background-800 rounded-lg shadow-lg">
		<thead>
			<tr class="bg-darker text-white uppercase text-xs leading-normal">
				<th class="px-2 py-1" scope="col" colspan="3">Top {count} Constellation</th>
			</tr>
		</thead>
		<tbody class="text-background-300 text-sm">
			{#each constellations as constellation (constellation.constellation_id)}
				...
				>
					<td class="px2 py-1">
						<img style="width: 32px; height: 32px;" src={`/map.png`} alt="Constellation: {constellation.name}" />
					</td>
					<td class="px-2 py-1">
						<div class="flex items-center">
							<div>
								<div class="text-primary-400">{constellation.name}</div>
							</div>
						</div>
					</td>
					<td class="px-2 py-1">
						<span class="ml-2">{constellation.count}</span>
					</td>
				</tr>
			{/each}
			<tr>
