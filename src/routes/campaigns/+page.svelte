        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center mb-6 relative">
          ...
          <div class="relative">
            ...
            <div class="flex items-center border border-gray-600 rounded-lg py-2 px-4 bg-gray-900 text-white">
              ...
              {/if}
              <input type="text" bind:value={locations[index].searchTerm} on:input={() => searchForLocation(index)} class="flex-1 bg-transparent focus:outline-none" placeholder="Search for location..." />
            </div>
              ...
              <div class="absolute bg-gray-800 rounded-lg shadow-lg -mt-1 w-full bottom-full mb-2">
                <ul class="max-h-64 overflow-y-auto">
                  {#each locations[index].searchResults as result, resultIndex}
                    <li class="flex items-center px-4 py-2 text-gray-300 cursor-pointer hover:bg-gray-700 {locations[index].selectedIndex === resultIndex ? 'bg-gray-600' : ''}" on:click={() => handleLocationSelect(index, result)}>
