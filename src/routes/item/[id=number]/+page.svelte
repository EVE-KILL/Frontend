                                ...
                                <tr class="hover:bg-gray-600 transition-colors duration-300 cursor-pointer" on:click={() => handleKillmailClick(kill.killmail_id)}>
                                    <td class="w-1/3 flex items-center space-x-2">
                                        <a href={`/kill/${kill.killmail_id}`}>
                                            <img src={`https://images.evetech.net/types/${kill.victim.ship_type_id}/render?size=64`} class="rounded" alt="{kill.victim.ship_name}" onerror="this.setAttribute('shipImageError', 'true')">
                                        </a>
                                        <a href={`/character/${kill.victim.character_id}`}>
                                            <img src={`https://images.evetech.net/characters/${kill.victim.character_id}/portrait?size=64`} class="rounded" alt="{kill.victim.character_name}">
                                        </a>
                                        <a href={`/corporation/${kill.victim.corporation_id}`}>
                                            <img src={`https://images.evetech.net/corporations/${kill.victim.corporation_id}/logo?size=64`} class="rounded" alt="{kill.victim.corporation_name}" onerror="this.onerror = null; this.src='https://images.evetech.net/alliances/1/logo?size=64'">
                                        </a>
                                        {#if kill.victim.alliance_id}
                                            <a href={`/alliance/${kill.victim.alliance_id}`}>
                                                <img src={`https://images.evetech.net/alliances/${kill.victim.alliance_id}/logo?size=64`} class="rounded" alt="{kill.victim.alliance_name}">
                                            </a>
                                        {/if}
                                    </td>
                                    <td class="w-2/3">
                                        <div class="text-sm">
                                            ...
                                            <span>{kill.victim.corporation_name}</span><br>
                                            {#if kill.victim.alliance_name}
                                                <span>{kill.victim.alliance_name}</span><br>
                                            {/if}
                                        </div>
                                    </td>
                                </tr>
